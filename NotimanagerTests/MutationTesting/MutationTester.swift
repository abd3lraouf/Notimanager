...
class MutationTester {
    ...
    private func backupFile(_ path: String) -> Bool {
        ...
        do {
            ...
            log("Error backing up file \(path): \(error)")
            return false
        }
    }
    ...
    private func restoreFile(_ path: String) -> Bool {
        ...
        // Restore from backup
        do {
            try fileManager.copyItem(atPath: backupPath, toPath: path)
            // Remove backup
            try? fileManager.removeItem(atPath: backupPath)
            return true
        } catch {
            log("Error restoring file \(path): \(error)")
            return false
        }
    }
...
class TestFailureObserver: NSObject, XCTestObservation {
    ...
    func testCase(_ testCase: XCTestCase, didFailWithDescription description: String, inFile filePath: String?, atLine lineNumber: Int) {
        let failureMessage = "\(testCase.name): \(description)"
