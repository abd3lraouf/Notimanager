...
class CommonTestHelpers {
    ...
    static func waitForCondition(
        ...
        _ = Timer.scheduledTimer(withTimeInterval: checkInterval, repeats: true) { timer in
            if condition() {
                timer.invalidate()
                expectation.fulfill()
        ...
        _ = XCTWaiter.wait(for: [expectation], timeout: timeout + 0.1)
    }
    ...
    static func createTempFile(content: String, fileExtension: String = "txt") -> URL {
        let tempDir = FileManager.default.temporaryDirectory
        let fileURL = tempDir.appendingPathComponent("test_\(UUID().uuidString).\(fileExtension)")
