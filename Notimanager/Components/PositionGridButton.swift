...
class PositionGridButton: NSView {
    ...
    private func setupContainer() {
        ...
        containerView.material = isSelected ? .selection : .titlebar
    ...
    private func updateContainerStyle() {
        ...
        containerView.layer?.borderColor = isSelected
            ...
            : Colors.separator.withAlphaComponent(0.3).cgColor

        // Add subtle background fill when not selected for glass effect
        if !isSelected {
            containerView.layer?.backgroundColor = NSColor.white.withAlphaComponent(0.05).cgColor
        } else {
