...
public struct NotificationTestData: TestData {
    ...
    public func validate() throws {
        ...
        // Validate position format - only 4 corner positions supported
        let validPositions = ["topLeft", "topRight", "bottomLeft", "bottomRight"]
        guard validPositions.contains(position) else {
            throw TestDataError.validationFailed("Invalid position: \(position). Must be one of: \(validPositions.joined(separator: ", "))")
        }
    }
}
